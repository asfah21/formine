...
@section('title', 'Hasil Compactor GSI')
        ...
        <div class="py-8 px-4 mx-auto max-w-5xl lg:py-16 !pb-40 !pt-28">
            ...
            <div class="p-5 text-center">
                <h3 class="mb-2 text-2xl font-extrabold tracking-tight leading-none md:text-2xl xl:text-2xl dark:text-white">
                    Tower <a class="font-extrabold text-blue-500 dark:text-blue-400 hover:underline">Lamp</a>
                </h3>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <form method="GET" action="{{ route('hasil-tl') }}" class="flex items-center">
                    ...
                    <div
                        ...
                        <div class="flex items-center space-x-3 w-full md:w-auto">
                            ...
                            <form method="GET" action="{{ route('hasil-tl') }}">
                            ...
                            @forelse ($tl as $mh)
                                <tr class="border-b dark:border-gray-700">
                                    <td class="px-4 py-3 text-center">
                                        {{ ($tl->currentPage() - 1) * $tl->perPage() + $loop->iteration }}</td>
                                    ...
                                    <td class="text-center align-middle">
                                        <a class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-semibold rounded-lg text-xs px-4 py-1"
                                            href="{{ route('detailTl.show', [$mh->nama_driver, $mh->tl_id]) }}">
                                            Lihat
                                        </a>
                                    </td>
                ...
                <div class="mt-4 py-6 px-5 flex flex-col md:flex-row justify-between items-center space-y-3 md:space-y-0">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-medium">{{ $tl->firstItem() }}</span>
                        to
                        <span class="font-medium">{{ $tl->lastItem() }}</span>
                        of
                        <span class="font-medium">{{ $tl->total() }}</span>
                        results
                    </span>
                    ...
                    <nav aria-label="Pagination">
                        <ul class="inline-flex -space-x-px space-x-1">
                            ...
                            @php
                                $start = max($tl->currentPage() - 1, 1);
                                $end = min($start + 2, $tl->lastPage());
                            @endphp
