...
<section class="min-h-screen bg-white dark:bg-gray-900 pt-28 md:pt-0">
    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        ...
        <div class="w-full p-6 bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md dark:bg-gray-800 dark:border-gray-700 sm:p-8">
            <h2 class="mb-1 text-l font-bold leading-tight tracking-tight text-gray-900 md:text-xl dark:text-white text-center">
                Masukkan Password untuk mengakses halaman stock
            </h2>

            <form class="mt-4 space-y-4 lg:mt-5 md:space-y-5" action="{{ route('stock.auth.submit') }}" method="POST">
                @csrf
                <div>
    ...
    {{-- <h2 class="mt-20">Masukkan Password untuk Mengakses Halaman Stok</h2>

    @if (session('error'))
        <p style="color: red;">{{ session('error') }}</p>
    @endif

    <form action="{{ route('stock.auth.submit') }}" method="POST">
        @csrf
