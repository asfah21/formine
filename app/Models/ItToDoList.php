...
class ItToDoList extends Model
{
    use HasFactory;

    protected $table = 'it_work_orders'; //Arahkan ke tabel db langsung
    ...
    public static function boot()
    {
        ...
        static::creating(function ($workOrder) {
            // Cari nomor terakhir yang diawali dengan "TD-"
            $lastTicket = self::where('ticket_number', 'LIKE', 'TD-%')
                ->latest('id')
                ->first();
            $lastNumber = $lastTicket ? intval(substr($lastTicket->ticket_number, -4)) : 0;
            $newNumber = str_pad($lastNumber + 1, 4, '0', STR_PAD_LEFT);
            $workOrder->ticket_number = 'TD-' . $newNumber;
        });
