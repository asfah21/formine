...
use App\Filament\Resources\DumptruckResource\Pages;
use App\Filament\Resources\DumptruckResource\RelationManagers;
...
class DumptruckResource extends Resource
{
    protected static ?string $model = Dumptruck::class;

    protected static ?string $navigationGroup = 'Formulir';

    protected static ?string $slug = 'dumptruck'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'Dump Truck';

    protected static ?string $navigationLabel = 'Dump Truck';
    ...
    {
        $columns = array_filter((new Dumptruck())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Dumptruck, tanpa filter pada approve atau status
        $query = Dumptruck::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailDt.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->dt_id,
                ])),
        ])
        ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListDumptrucks::route('/'),
            'create' => Pages\CreateDumptruck::route('/create'),
            'edit' => Pages\EditDumptruck::route('/{record}/edit'),
        ];
    }
}
