...
class ExcavatorResource extends Resource
{
    ...
    protected static ?string $slug = 'excavator'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'Excavator';

    protected static ?string $navigationLabel = 'Excavator';
    ...
    {
        $columns = array_filter((new Exca())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Exca, tanpa filter pada approve atau status
        $query = Exca::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailExca.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->ex_id,
                ])),
        ])
        ->bulkActions([
