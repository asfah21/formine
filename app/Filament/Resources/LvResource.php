...
use App\Filament\Resources\LvResource\Pages;
use App\Filament\Resources\LvResource\RelationManagers;
use App\Models\Lv;
use Filament\Forms;
...
class LvResource extends Resource
{
    protected static ?string $model = Lv::class;

    protected static ?string $navigationGroup = 'Formulir';

    protected static ?string $slug = 'light-vehicle'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'Light Vehicle';

    protected static ?string $navigationLabel = 'Light Vehicle';
    ...
    {
        $columns = array_filter((new Lv())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Dumptruck, tanpa filter pada approve atau status
        $query = Lv::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailLv.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->lv_id,
                ])),
        ])
        ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListLvs::route('/'),
            'create' => Pages\CreateLv::route('/create'),
            'edit' => Pages\EditLv::route('/{record}/edit'),
        ];
    }
}
