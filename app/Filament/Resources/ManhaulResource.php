...
use App\Filament\Resources\ManhaulResource\Pages;
use App\Filament\Resources\ManhaulResource\RelationManagers;
...
class ManhaulResource extends Resource
{
    protected static ?string $model = Manhaul::class;

    protected static ?string $navigationGroup = 'Formulir';

    protected static ?string $slug = 'manhaul'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'Manhaul';

    protected static ?string $navigationLabel = 'Manhaul';
    ...
    {
        $columns = array_filter((new Manhaul())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Dumptruck, tanpa filter pada approve atau status
        $query = Manhaul::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailMh.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->mh_id,
                ])),
        ])
        ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListManhauls::route('/'),
            'create' => Pages\CreateManhaul::route('/create'),
            'edit' => Pages\EditManhaul::route('/{record}/edit'),
        ];
    }
}
