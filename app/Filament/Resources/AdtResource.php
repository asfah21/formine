...
use App\Filament\Resources\AdtResource\Pages;
use App\Filament\Resources\AdtResource\RelationManagers;
use App\Models\Adt;
...
class AdtResource extends Resource
{
    protected static ?string $model = Adt::class;

    protected static ?string $navigationGroup = 'Formulir';

    protected static ?string $slug = 'adt'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'ADT';

    protected static ?string $navigationLabel = 'ADT';
    ...
    {
        $columns = array_filter((new Adt())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Dumptruck, tanpa filter pada approve atau status
        $query = Adt::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailAdt.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->adt_id,
                ])),
        ])
        ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListAdts::route('/'),
            'create' => Pages\CreateAdt::route('/create'),
            'edit' => Pages\EditAdt::route('/{record}/edit'),
        ];
    }
}
