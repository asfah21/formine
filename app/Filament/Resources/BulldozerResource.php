...
use App\Filament\Resources\BulldozerResource\Pages;
use App\Filament\Resources\BulldozerResource\RelationManagers;
use App\Models\Bulldozer;
...
class BulldozerResource extends Resource
{
    protected static ?string $model = Dozer::class;

    protected static ?string $navigationGroup = 'Formulir';

    protected static ?string $slug = 'bulldozer'; // 'huruf kecil dan tanpa spasi'

    protected static ?string $label = 'Bulldozer';

    protected static ?string $navigationLabel = 'Bulldozer';
    ...
    {
        $columns = array_filter((new Dozer())->getFillable(), fn($column) => $column);

        // Query untuk menampilkan semua record Dumptruck, tanpa filter pada approve atau status
        $query = Dozer::query()->orderBy('date', 'desc');
        ...
        ->actions([
            ...
            Action::make('view')
                ...
                ->icon('heroicon-s-eye')
                ->color('success')
                ->url(fn($record) => route('detailBd.show', [
                    'name' => $record->nama_driver,
                    'aptnumx' => $record->bd_id,
                ])),
        ])
        ->bulkActions([
    ...
    {
        return [
            'index' => Pages\ListBulldozers::route('/'),
            'create' => Pages\CreateBulldozer::route('/create'),
            'edit' => Pages\EditBulldozer::route('/{record}/edit'),
        ];
    }
}
